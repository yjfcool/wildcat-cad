...
void InitGeometry(void) {
	...
	//Add a control point into the points layer
	WCControlPoint* cp = new WCControlPoint(0.0, 0.0, 0.0);
	cpLayer->RegisterGeometricObject(cp);
	...
	sLayer->RegisterGeometricObject(surface);
	cpLayer->RegisterGeometricObject(surfCP);
	...
	WPFloat data[] = {
		...
		1.3333333,	1.0,		-1.0, 1.0,
		2.0,		1.0,		0.0, 1.0
	};	
	surfCP = WCControlPoint::CollectionFromArray(16, data);
	surface = new WCNurbsSurface(2, 2, 4, 4, surfCP, WCNurbsMode::Default(), WCNurbsMode::Default());
	std::cout << surface << std::endl;
	surface->LevelOfDetail(30, 30);	
	sLayer->AddObject(surface);
	cpLayer->IsVisible(false);
//	cpLayer->AddObject(surfCP);	
	
	/*** End Test ***/
	
	surface->Color( WCColor(0.75, 0.75, 0.5, 1.0) );
	sLayer->RenderProgram(WCShaderManager::ProgramFromName("scn_gooch"));
}
...
void KeyPress(unsigned char k, int x, int y) {
	switch(k) {
		...
		//Color switches
		case '6': surface->Color( WCColor(0.75, 0.45, 0.45, 0.2) ); break;
...
int main(int argc, char **argv) {
	...
	InitScene();
	InitGeometry();
	//Show some info about the scene
