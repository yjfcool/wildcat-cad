	...
	for (jr.x=1; jr.x<=numParams.x; jr.x++) {
		...
		if (jr.x == numParams.x-1) {
			...
			for (jr.y=0; jr.y<numParams.x; jr.y++) {
				...
				bvD[numParams.x + jr.y + 1] =  savedTemp.x - savedTemp.y;
				//Swap right side to left
				savedTemp.x = savedTemp.y;
			}
			//Save the last der-basis
			bvD[numParams.x * 2 + 1] = savedTemp.x;
		}
	}
}
...
//			derX += (controlPoints[index].xyz * basisValuesU[numParamsU.x+1+i] * basisValuesV[j]);
//			derY += (controlPoints[index].xyz * basisValuesU[i] * basisValuesV[numParamsV.x+1+j]);
		}
	}
	//Set the position (don't forget the w divide) and emit
//	pos = pos / w;
	gl_FragData[0] = vec4(pos, 1.0);	//Calculate the normal
//	gl_FragData[0] = vec4(inVert);
//	gl_FragData[0] = vec4(1.0, 2.0, 3.0, 4.0);
//	gl_FragData[0] = vec4( span[0], 9, 9, 9 );
//	gl_FragData[0] = vec4(span[0], span[1], 3.0, 4.0);
